...
public class VerifyEmail {
		...
		try {
			OUTERLOOP: for (int k = 1; k <= 10; k++) {
				...
				if (emailDomain.equalsIgnoreCase("gmail")) {
					store.connect(PropertyFileUtils.getPropValuesFromConfig(Constants.EMAIL_PROPERTIES_FILE,
							"gmail.email.host"), emailId, password);
				} else if (emailDomain.equalsIgnoreCase("outlook")) {
					store.connect(PropertyFileUtils.getPropValuesFromConfig(Constants.EMAIL_PROPERTIES_FILE,
							"outlook.email.host"), emailId, password);
				} else {
	 ...
	 *             Signals that an I/O exception has occurred.
	 */
	public static void emailVerification(String finalFileName, String emailId, String password)
		...
		try {
			OUTERLOOP: for (int k = 1; k <= 10; k++) {
				 ...
				 * code works. If we use email id with innominds,com/gmail.com
				 * based on the domains the email verification works in the
				 * respected mail accounts.
				 */
				...
				for (int i = 1; i <= 20; i++) {
					...
					if (message2.getSubject().equalsIgnoreCase("Uploaded File Matrix")
							&& message2.getContent().toString().trim().contains(finalFileName)) {
						try {
							if (message2.getContent().toString().trim().contains("Error Details")) {
								errorFound = true;
								// System.out.println("Email received with
								// error");
								/*
								 * System.out .println(
				...
				if (k == 10) {
					Assert.fail(
							"Upload File Matrix Mail not received even after waiting for 2 min 30sec, hence failing the test case");
				}
			}
		} catch (MessagingException e) {
			System.out.println("Error: " + e);
	 ...
	 */
	public static void getConfirmationEmail(String FirstName, String LastName, String emailId, String password)
			throws FileNotFoundException {
		int time = 0;
		System.out.println();
		System.out.println("ON!Track Activation Mail Email verification started......");
		System.out.println();
		errorFoundNew = false;
		try {
			OUTERLOOP: for (int k = 1; k <= 10; k++) {
				...
				if (emailDomain.equalsIgnoreCase("outlook")) {
					store.connect(PropertyFileUtils.getPropValuesFromConfig(Constants.EMAIL_PROPERTIES_FILE,
							"outlook.email.host"), 995, emailId, password);
				} else if (emailDomain.equalsIgnoreCase("gmail")) {
				...
				for (int i = 1; i <= 20; i++) {
					...
					if (message2.getSubject().equalsIgnoreCase("ON!Track Activation Mail")
							&& content.toString().trim().contains(FirstName)
							&& content.toString().trim().contains(LastName)) {
						System.out.println();
						System.out.println("ON!Track Activation Email Mail Received and Verified!!!");
						System.out.println();
						...
						System.out.println("Link from the mail is =============> " + confirmationEmailUrl);
						String readMailIdFromConfirmationEmail = readMailid(contentStr);
						System.out
								.println("User id from the mail is =============> " + readMailIdFromConfirmationEmail);
						System.out.println();
				...
				if (k == 10) {
					if (confirmationEmailUrl.isEmpty()) {
						System.out.println(
								"Expected ON!Track Activation Mail not received even after waiting for 2 min 30 sec, so confirmation link will be Empty");
						errorFoundNew = true;
						Assert.fail(
								"Expected ON!Track Activation Mail not received even after waiting for 2 min 30 sec, so confirmation link will be Empty");
						...
						System.out.println(
								"Expected ON!Track Activation Mail not received even after waiting for 2 min 30 sec, hence failing the test case");
						errorFoundNew = true;
						Assert.fail(
								"Expected ON!Track Activation Mail not received even after waiting for 2 min 30 sec, hence failing the test case");
						break OUTERLOOP;
	 ...
	 * @throws FileNotFoundException
	 */
	public static void getEmailChangeConfirmationEmail(String FirstName, String LastName, String newemailid,
			String emailId, String password) throws FileNotFoundException {
		int time = 0;
		String old_email_emailchangecontent = "Your ON!Track email was changed to " + newemailid + ".";
		contentFound = false;
		System.out.println();
		System.out.println("ON!Track Notification Mail Email verification started......");
		System.out.println();
		try {
			OUTERLOOP: for (int k = 1; k <= 7; k++) {
				...
				for (int i = 1; i <= 20; i++) {
					...
					if (message2.getSubject().equalsIgnoreCase("ON!Track Notification Mail")
							&& content.toString().trim().contains(FirstName)
							&& content.toString().trim().contains(LastName)
							&& content.toString().trim().contains(old_email_emailchangecontent)) {
						contentFound = true;
						System.out.println();
						System.out.println("ON!Track Notification Mail Email Received and Verified!!!");
						System.out.println();
						System.out.println("ON!Track Notification Mail Received to old Email ID \"" + emailId
								+ "\" and has Expected new \"" + newemailid + "\" details");
						break OUTERLOOP;
					}
				}
				folder.close(false);
				store.close();
				if (k == 7) {
					System.out.println("Expected Content \"" + old_email_emailchangecontent
							+ "\" Not Received to old Email ID \"" + emailId + "\" even after waiting for 2 minutes");
					Assert.fail("Expected Content \"" + old_email_emailchangecontent
							+ "\" Not Received to old Email ID \"" + emailId + "\" even after waiting for 2 minutes");
					break OUTERLOOP;
				}
			}
	 ...
	 * @throws FileNotFoundException
	 */
	public static void getConfirmationEmailChangePassword(String FirstName, String LastName, String emailId,
		...
		try {
			OUTERLOOP: for (int k = 1; k <= 10; k++) {
				...
				for (int i = 1; i <= 20; i++) {
					...
					if (message2.getSubject().equalsIgnoreCase("ON!Track - your new password")
							&& content.toString().trim().contains(FirstName)
							&& content.toString().trim().contains(LastName)) {
						System.out.println("Email received and verified.......");
						String contentStr = content.toString().trim();
						...
						System.out.println("Link from the mail is ===========================================> "
								+ confirmationEmailUrl);
						String readMailIdFromConfirmationEmail = readMailid(contentStr);
						System.out.println("User id from the mail is ===========================================> "
				...
				if (k == 10) {
					if (confirmationEmailUrl.isEmpty()) {
						System.out.println(
								"ON!Track Activation Mail not received even after waiting for 2 min 30 sec, so confirmation link will be Empty");
						Assert.fail(
								"ON!Track Activation Mail not received even after waiting for 2 min 30 sec, so confirmation link will be Empty");
						break OUTERLOOP;
					} else {
						System.out.println(
								"ON!Track Activation Mail not received even after waiting for 2 min 30 sec, hence failing the test case");
						Assert.fail(
								"ON!Track Activation Mail not received even after waiting for 2 min 30 sec, hence failing the test case");
						break OUTERLOOP;
					}
