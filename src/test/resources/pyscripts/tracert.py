...
from com.innominds.team.listeners import ResultsHolder
...
for number,ip in enumerate(data_input):
 ...
 try:
    ...
    p = Popen("tracert -d -h 5 -w 5 %s" % ip,stdout=PIPE)
    out=p.communicate()[0]
    # if isinstance(out, bytes):
    #     out = str(out, 'utf-8')
    # if type(out) is str:
    #trace = re.search(r'.*Request timed out.*', out)
    if re.search(r'.*Request timed out.*', out):
        print("\n###### Tracert output ########\n", out)
        ...
        print("\n----Tracert unsuccessful for %s----\n" % ip)
        msg="Unable to resolve target system name"
        Test_Status="Fail"
    else:
        print("\n###### Tracert output########\n", out)
